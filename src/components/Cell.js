...
import {
  decrementMarked, incrementMarked, setAllMarked, setMaskField, setNewField
} from '../store/reducers/minesReducer';
...
const Cell = ({id}) => {
  ...
  const newField = isFirstMove
    ? generateNewField(
      minesfieldHeight, minesfieldWidth, minesNumber, Math.floor(id / minesfieldHeight), id % minesfieldWidth
    )
    : field.slice();
  const onCellClick = () => {
    if (isFirstMove) {
